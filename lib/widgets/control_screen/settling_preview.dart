...
// (CustomizationInteractionMode.settlingWidget): tweens the SAME preview
// visual (CatalogPreviewStage — never a different-looking widget) from
// wherever the finger released ([startRect]) into the validated grid
// rectangle LayoutEditController already found ([endRect]), animating both
// position AND size together so the widget never teleports.
//
...
// and that Stack fills the whole screen from its own (0, 0), so global
// coordinates double as Positioned coordinates here exactly like
// PlacementCancelBar's own Positioned(top: 0, left: 0, ...) already assumes.
//
// [onSettled] — LayoutEditController.commitSettledPlacement — adds the real
// ButtonConfig to the draft and flips interactionMode back to editing in the
// SAME notifyListeners() call that removes this widget from the tree, so
// there is no frame where neither the floating preview nor the real grid
// button is visible, and at most one frame where both are (at the identical
// final rectangle, so indistinguishable from a clean handoff).
// ─────────────────────────────────────────────────────────────────────────────
