...
class _IndustrialMultiZoneSliderState extends State<IndustrialMultiZoneSlider>
  ...
  Widget _endpointLabel(String text, IconData icon, bool active) {
    final activeColor = active ? _trackColor : AppColors.darkText;
    ...
    return Expanded(
      child: ControlButtonLabelIcon(
        ...
        iconColor: active ? _trackColor : AppColors.darkTextMuted,
...
class _TrackPainter extends CustomPainter {
  ...
  void _paintZoneLabel(Canvas canvas, String text, double centerX, double top) {
    ...
    final badgeRect = Rect.fromCenter(
      center: Offset(centerX, top + textPainter.height / 2.0 + verticalPadding),

      width: textPainter.width + horizontalPadding * 2.0,

      height: textPainter.height + verticalPadding * 2.0,
    );

    canvas.drawRRect(
      RRect.fromRectAndRadius(badgeRect, const Radius.circular(3)),

      Paint()..color = AppColors.darkBg.withAlpha(150),
    );
    ...
    textPainter.paint(
      ...
      Offset(
        badgeRect.left + horizontalPadding,
        badgeRect.top + verticalPadding,
      ),
    );
  }
